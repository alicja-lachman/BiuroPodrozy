...
void Laczona::drukujLaczona(BiuroPodrozy *biuro){
    ...
    plik<<biuro->podajIlosc()<<". "<<"Wycieczka laczona "<<
    objazd.getNazwa()<<" Kraje: ";
    for (const string& x: objazd.getListaKrajow()) plik<< x<< " ";
    plik<<"Miasta: ";
    for (unsigned int i=0; i<objazd.getListaMiastRozmiar(); i++) plik<<objazd.getListaMiast(i)<<" ";
    plik<<" Termin: "<<
